...
object AppUpdater {
    ...
    private const val APK_CACHE_NAME = "hairclinic-update.apk"
    ...
    fun resolveDownloadUrl(baseUrl: String, raw: String): String {
        val path = when {
            raw.isBlank() -> "/download/hairclinic.apk"
            raw.startsWith("/") -> raw
            else -> {
                val uri = Uri.parse(raw)
                if (isLoopbackHost(uri.host)) {
                    uri.path?.takeIf { it.isNotBlank() } ?: "/download/hairclinic.apk"
                } else {
                    return raw
                }
            }
